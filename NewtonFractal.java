...
class NewtonFractal {
    ...
    public int findRoot(Complex root) {
        // You need to fill in this function.
        for(int i = 0; i < roots.size(); i++){
            //checks if two roots are equal with regard to tolerance
    ...
    public void createFractal(boolean colorIterations) {
        ...
        //2. Iterate over each pixel (j,k)
        Complex root0,root1;
        int numIter;
        //3. Apply Newton-Raphson method
        for(int j = 0; j<NUMPIXELS; j++){
          for(int k = 0; k<NUMPIXELS; k++){
            root0 = pixelToComplex(j,k);
            this.iterator.iterate(root0);

            root1 = iterator.getRoot();
            numIter = iterator.getNumIterations();

        //4. Add to roots list, if this root not already present
            if(findRoot(root1) == -1){
              roots.add(root1);
            }

        //5. Colour pixel
          this.colorPixel(j,k, findRoot(root1), numIter);
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(-1), new Complex(1),
                                          new Complex(), new Complex(-1),
                                          new Complex(), new Complex(1)};
